...
import { HiOutlineTrash, HiOutlineDocumentAdd, HiOutlineMinus, HiOutlinePlus } from "react-icons/hi";
import { FaRegEdit } from "react-icons/fa";
import { getAllRegion, getAllSubRegion } from '../../redux/locationReducer';
import { modifyProduct, getProductsByFilter } from '../../redux/productReducer';
...
function CreateProduct(props) {
    ...
    const [changeState, setChangeState] = useState({
        product_name: "unchange",
        producer_name: "unchange",
        product_type: "unchange",
        // for wine
        wine_color: "unchange",
        wine_type: "unchange",
        fortified_wine_type: "unchange",
        variaty: "unchange",
        barrel_type: "unchange",
        wine_acid: "unchange",
        // for spirit
        spirit_sub_type: "unchange",
        spirit_type: "unchange",
        //for beer
        beer_type: "unchange",
        // for wine & spirit
        vintage: "unchange",
        taste: "unchange",
        //for all
        bottle_size: "unchange",
        bottle_image: "unchange",
        article: "unchange",
        // for basic
        country: "unchange",
        region: "unchange",
        ...
        food: "unchange",
        allergy: "unchange",
        closure_type: "unchange",
        residual_sugar: "unchange",
    })
    ...
    const addGrapeVariaty = () => {
        ...
        if (grape) {
            ...
            if (!flag) {
                ...
                //for company product change state
                changeState['variaty'] = "change";
    ...
    const saveProduct = async () => {
        ...
        if (validate_result) {
            ...
            const { producer_name, product_name, product_type, vintage, bottle_size, article,
                variaty, description, alc_vol, aroma, food, barrel_type, taste, residual_sugar, wine_acid,
                closure_type, allergy } = productData;
            // const { producer } = props;

            var searchText = createProductSearchText(productData, { name: producer_name }, product_type);
            ...
            // const { role } = userInfo;
            formData.append("modified_by", props.condition.user_id);
            formData.append("role", props.condition.role);
            formData.append("company", props.condition.company);
            formData.append("isGlobal", props.condition.isGlobal);
            formData.append("user_id", props.condition.user_id);
            formData.append("change_state", JSON.stringify(changeState));
            // formData.append("producer_name", producer_name);
            if (product_type.value == 0) { //wine
            ...
            } else if (product_type.value == 1) { // spirit
                formData.append("product_name", product_name);
                vintage && formData.append("vintage", vintage.value);
                formData.append("bottle_size", bottle_size.value);
                article && formData.append("article", article);
                ...
                allergy && formData.append("allergy", JSON.stringify(allergy_send));
                searchText && formData.append("search_field", searchText);
            } else if (product_type.value == 2) { // beer
                formData.append("product_name", product_name);
                formData.append("bottle_size", bottle_size.value);
                article && formData.append("article", article);
                // basic
            ...
            const res = await dispatch(modifyProduct(props.product._id, formData));
            if (res != false) {
    ...
    const openModal = () => {
        ...
        if (product) {
            setProduct(product);
            const { producer, product_name, wine_color, product_type, vintage, category, sub_category, article_nr, grape_variety, bottle_image,
                country, region, sub_region, bottle_size, description_text, alc_vol, aroma, food_pairing, barrel_type, taste, residual_sugar, wine_acid,
            ...
            setProductData({
                ...
                // for wine & spirit
                vintage: { value: vintage, label: vintage ? vintage : "N/A" },
                //for all
                bottle_size: bottle_size ? bottlesize_options[bottle_size._id] : "",
                article: article_nr ? article_nr : "", //article_nr,
                // for basic
                ...
                taste: taste >= 0 ? { value: taste, label: product_type.sub[category - 1].taste[taste] } : "",
    ...
    const setBottle = (data) => {
        changeState["bottle_image"] = "change";

        setChangeState({ ...changeState });
        setBottleImages(data);
    }
    const closeModal = () => {
        swal.fire({
            ...
            >
                ...
                <Modal.Footer>
                    <div className="flex flex-wrap items-center gap-4 m-auto">
                        ...
                        <div>
                            <Button onClick={() => saveProduct()} className="bg-sitebg-300">
                                {t("save_product")}
